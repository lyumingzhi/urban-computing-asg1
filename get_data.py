		...
		for path_file in path_data_files:
			...
			for entry in path_data:
				...
				if entry[1] == 'TYPE_ACCELEROMETER_UNCALIBRATED':
					self.acce_uncali[path_file].append([int(entry[0]), float(entry[2]), float(entry[3]), float(entry[4])])
					continue
				...
				if entry[1] == 'TYPE_GYROSCOPE_UNCALIBRATED':
					self.gyro_uncali[path_file].append([int(entry[0]), float(entry[2]), float(entry[3]), float(entry[4])])
					continue
				...
				if entry[1] == 'TYPE_MAGNETIC_FIELD_UNCALIBRATED':
					self.magn_uncali[path_file].append([int(entry[0]), float(entry[2]), float(entry[3]), float(entry[4])])
					continue
